...
@Mod(modid = EZAutoScript.MODID, name = EZAutoScript.NAME, version = EZAutoScript.VERSION, clientSideOnly = true)
public class EZAutoScript {
    public static final String MODID = "ezautoscript";
    public static final String NAME = "EZAutoScript";
    ...
    public void onClientChat(ClientChatEvent event) {
        ...
        if (!enabled && msg.startsWith(prefix)) {
            ...
            if (isKnown && !allowedWhileDisabled) {
                mc.player.sendMessage(new TextComponentString("\u00A77[\u00A7fEZAutoScript\u00A77] \u00A7cMod is currently disabled (Use " + prefix + "enable)."));
        ...
        if (msg.equalsIgnoreCase(prefix + "set")) {
            mc.player.sendMessage(new TextComponentString("\u00A77[\u00A7fEZAutoScript\u00A77] \u00A7cIncorrect Usage! Please use " + prefix + "set {block(s)}."));
            event.setCanceled(true);
            return;
        }
        ...
        if (lmsg.equals((prefix + "enable").toLowerCase())) {
            enabled = true;
            mc.player.sendMessage(new TextComponentString("\u00A77[\u00A7fEZAutoScript\u00A77] \u00A7aEnabled."));
            ...
            mc.player.sendMessage(new TextComponentString("\u00A77[\u00A7fEZAutoScript\u00A77] \u00A7cDisabled."));
            event.setCanceled(true);
            ...
            if (!setItems.isEmpty()) {
                new Thread(() -> {
                    try {
                        ...
                        mc.player.sendMessage(new TextComponentString("\u00A77[\u00A7fEZAutoScript\u00A77] \u00A7aStarting Baritone."));
                    } catch (Exception e) {
                        e.printStackTrace();
                    }
                }).start();
            } else {
                mc.player.sendMessage(new TextComponentString("\u00A77[\u00A7fEZAutoScript\u00A77] \u00A7cNo items specified. Set blocks first."));
            }
            event.setCanceled(true);

        } else if (lmsg.equals((prefix + "stop").toLowerCase())) {
            AutoScript.sendChat("#stop");
            mc.player.sendMessage(new TextComponentString("\u00A77[\u00A7fEZAutoScript\u00A77] \u00A7cStopping Baritone."));
            event.setCanceled(true);
                ...
                try {
                    ...
                    if (blocks == null || blocks.trim().isEmpty()) {
                        mc.addScheduledTask(() -> {
                            if (mc.player != null) {
                                ...
                                mc.player.sendMessage(new TextComponentString("\u00A77[\u00A7fEZAutoScript\u00A77] \u00A7cNo setBlock configured. Use " + dynPrefix + "set {block(s)} first."));
                            }
                        });
                        return;
                    }
                    ...
                    mc.addScheduledTask(() -> {
                        if (mc.player != null) {
                            ...
                            mc.player.sendMessage(new TextComponentString("\u00A77[\u00A7fEZAutoScript\u00A77] \u00A7aSet: " + String.join(", ", displayItems)));
                        }
                    });
                    Thread.sleep(2000);
            ...
            mc.player.sendMessage(new TextComponentString("\u00A7cEZAutoScript \u00A7f| \u00A77v1.1.1 \u00A7f| \u00A76by ArkOfReis"));
            mc.player.sendMessage(new TextComponentString("\u00A77------------------------------------------------"));
            mc.player.sendMessage(new TextComponentString("\u00A7a" + prefix + "enable \u00A7f| \u00A77Enables EZAutoScript"));
            mc.player.sendMessage(new TextComponentString("\u00A7a" + prefix + "disable \u00A7f| \u00A77Disables EZAutoScript"));
            ...
            mc.player.sendMessage(new TextComponentString("\u00A77------------------------------------------------"));
            mc.player.sendMessage(new TextComponentString("\u00A7cEZAutoScript \u00A7f| \u00A77v1.1.1 \u00A7f| \u00A76Mod Configuration"));
            mc.player.sendMessage(new TextComponentString("\u00A77------------------------------------------------"));
            ...
            mc.player.sendMessage(new TextComponentString("\u00A77[\u00A7fEZAutoScript\u00A77] \u00A7eUse base commands to change values:"));
            ...
            if (remainder.toLowerCase().startsWith("set ")) {
                ...
                mc.player.sendMessage(new TextComponentString("\u00A77[\u00A7fEZAutoScript\u00A77] \u00A7aautoWhisper set."));
                event.setCanceled(true);
            }
            else if (remainder.isEmpty()) {
                String msgText = ConfigManager.getAutoWhisper();
                if (msgText == null || msgText.trim().isEmpty()) {
                    mc.player.sendMessage(new TextComponentString("\u00A77[\u00A7fEZAutoScript\u00A77] \u00A7cSet a message first."));
                } else if (mc.getConnection() == null) {
                    mc.player.sendMessage(new TextComponentString("\u00A77[\u00A7fEZAutoScript\u00A77] \u00A7cNot connected."));
                } else {
                    mc.player.sendMessage(new TextComponentString("\u00A77[\u00A7fEZAutoScript\u00A77] \u00A7eWhispering to everyone..."));
                    new Thread(() -> {
                        try {
                            ...
                            mc.addScheduledTask(() -> {
                                if (mc.player != null) {
                                    mc.player.sendMessage(new TextComponentString("\u00A77[\u00A7fEZAutoScript\u00A77] \u00A7aSent " + sent + " DM(s)."));
                                }
                            });
                ...
                mc.player.sendMessage(new TextComponentString("\u00A77[\u00A7fEZAutoScript\u00A77] \u00A7eUsage: "
            ...
            if (ConfigManager.setCommandPrefix(arg)) {
                mc.player.sendMessage(new TextComponentString("\u00A77[\u00A7fEZAutoScript\u00A77] \u00A7amodPrefix set to '\u00A7f" + ConfigManager.getCommandPrefix() + "\u00A7a'."));
            } else {
                mc.player.sendMessage(new TextComponentString("\u00A77[\u00A7fEZAutoScript\u00A77] \u00A7cPlease enter a character/symbol."));
            }
            event.setCanceled(true);
            ...
            if (ConfigManager.setGoToGround(arg)) {
                mc.player.sendMessage(new TextComponentString("\u00A77[\u00A7fEZAutoScript\u00A77] \u00A7agoToGround set to \u00A7f" + (ConfigManager.getGoToGround() ? "true" : "false") + "\u00A7a."));
            } else {
                mc.player.sendMessage(new TextComponentString("\u00A77[\u00A7fEZAutoScript\u00A77] \u00A7cPlease enter a valid bool."));
            }
            event.setCanceled(true);
            ...
            if (ConfigManager.setShulkerColor(arg)) {
                mc.player.sendMessage(new TextComponentString("\u00A77[\u00A7fEZAutoScript\u00A77] \u00A7ashulkerColor set to \u00A7f" + ConfigManager.getShulkerColorDisplay() + "\u00A7a."));
            } else {
                mc.player.sendMessage(new TextComponentString("\u00A77[\u00A7fEZAutoScript\u00A77] \u00A7cPlease enter a valid color."));
            ...
            event.setCanceled(true);
                ...
                if (ConfigManager.setIgnoreStackedShulkers(arg)) {
                    mc.player.sendMessage(new TextComponentString("\u00A77[\u00A7fEZAutoScript\u00A77] \u00A7aignoreStackedShulkers set to \u00A7f" + (ConfigManager.getIgnoreStackedShulkers() ? "true" : "false") + "\u00A7a."));
                } else {
                    mc.player.sendMessage(new TextComponentString("\u00A77[\u00A7fEZAutoScript\u00A77] \u00A7cPlease enter a valid bool."));
                }
                event.setCanceled(true);
            ...
            if (lmsg.startsWith((prefix + "set ").toLowerCase())) {
                ...
                if (!argsPart.isEmpty()) {
                    ...
                    mc.player.sendMessage(
                            new TextComponentString("\u00A77[\u00A7fEZAutoScript\u00A77] \u00A7aSet: " + String.join(", ", displayItems))
                    );
                } else {
        ...
        if (!event.isCanceled() && msg.startsWith(prefix)) {
            mc.player.sendMessage(new TextComponentString("\u00A77[\u00A7fEZAutoScript\u00A77] \u00A7cIncorrect Usage. Please use " + prefix + "help!"));
            event.setCanceled(true);
            return;
        }
    }
                ...
                if (mc.player != null && mc.world != null) {
                    ...
                    mc.addScheduledTask(() -> {
                        if (!hasShownJoinMessage && mc.player != null) {
                            mc.player.sendMessage(new TextComponentString("\u00A77[\u00A7fEZAutoScript\u00A77] \u00A7fSuccessfully initialised. Mod is \u00A7cdisabled."));
                            String pfx = ConfigManager.getCommandPrefix();
                            mc.player.sendMessage(new TextComponentString("\u00A77[\u00A7fEZAutoScript\u00A77] \u00A7aSet a block (" + pfx + "set) before starting."));
                            mc.player.sendMessage(new TextComponentString("\u00A77[\u00A7fEZAutoScript\u00A77] \u00A76Your prefix is: " + pfx));
                ...
                try {
                    ...
                    if (enabled && !setItems.isEmpty()) {
                        ...
                        if (spawnY == 256) {
                            mc.addScheduledTask(() -> {
                                if (mc.player != null) {
                                    mc.player.sendMessage(new TextComponentString("\u00A77[\u00A7fEZAutoScript\u00A77] \u00A7eDetected Y = 256. Moving to Y=250."));
                                }
                            });
                            ...
                            if (!reachedTarget) {
                                mc.addScheduledTask(() -> {
                                    if (mc.player != null) {
                                        mc.player.sendMessage(new TextComponentString("\u00A77[\u00A7fEZAutoScript\u00A77] \u00A7cCould not reach Y=250. Suiciding."));
                                    }
                                });
                                if (enabled) {
                                    AutoScript.sendChat("/kill");
                        ...
                        mc.addScheduledTask(() -> {
                            if (mc.player != null) {
                                mc.player.sendMessage(new TextComponentString("\u00A77[\u00A7fEZAutoScript\u00A77] \u00A7aRunning."));
                            }
                        });
    ...
    private static void enforceWorldBorder(Minecraft mc) {
        ...
        if (outside) {
            ...
            if (now - lastBoundaryWarnMs > 3000L) {
                mc.player.sendMessage(new TextComponentString("\u00A77[\u00A7fEZAutoScript\u00A77] \u00A7eWorld border reached. Stay within X/Z [-1000, 1000]."));
                AutoScript.sendChat("#stop");
